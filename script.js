...
$('#orderForm').onsubmit=async e=>{
  ...
  try{const r=await submit('orders',{customer_name:d.customer_name,phone:d.phone,requested_date:d.date,notes:d.notes,items:cart,total_amount:total,status:'new'});if(r.demo){$('#orderStatus').textContent='Demo mode is active.';return}const id=r.data?.[0]?.id;if(!id)throw Error('Order ID was not returned');const code=orderCode(id);cart=[];saveCart();e.target.reset();closeModal('cartModal');const success=$('#successModal');$('#successOrderCode').textContent=code;$('#successOrderLink').href=`track.html?id=${id}`;success.classList.add('show');success.setAttribute('aria-hidden','false');}
