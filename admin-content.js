...
const state = { settings: null, gallery: [] };
...
function renderImages() {
  ...
  $('heroUrl').textContent = hero || 'No image selected';
  $('storyUrl').textContent = story || 'No image selected';
}
...
function renderGallery() {
  ...
  list.innerHTML = state.gallery.map((url, index) => `
    <article class="gallery-item">
      <img src="${esc(url)}" alt="Gallery photo ${index + 1}" loading="lazy">
...
$('galleryList').addEventListener('click', async event => {
  ...
  try {
    ...
    if (action === 'remove') {
      ...
      const patch = { gallery_urls: state.gallery };
...
$('galleryUpload').addEventListener('change', async event => {
  ...
  try {
    ...
    for (const file of files) {
      ...
      const path = `site-gallery/${Date.now()}-${Math.random().toString(36).slice(2)}-${safeName}`;
    ...
    await saveSettings({ gallery_urls: state.gallery });
    event.target.value = '';
    message(`${uploaded.length} photo${uploaded.length === 1 ? '' : 's'} added to the gallery.`, 'ok');
  } catch (error) {
    console.error(error);
